...
checkOp sigs = aux -- TODO(anon): What about TagJumpdest, TagJump, TagJumpi, TagPush
...
checkProcedure ps (Procedure name pre post ops) = consError name $ do checkStart
    ...
    where sigs = signatures ps
          ...
          checkReturns = sequence_ $ zipWith consError [1..] $ map aux ops'
              where aux (ProcedureReturn, S pre, _) = case (pre, post) of --TODO(anon): procedure can return Ø
...
procCall = Procedure "call" [V "gas", V "to", V "value", V "in offset", V "in size", V "out offset", V "out size"] (S [V "success"]) $
    ...
    -- TODO(anon): This might be a good place to check that value doesn't exceed mc.balance. Ask anon what he thinks
    ...
    -- If this is a call to a precompile, that's all we need to do. (assuming that precompiles don't accept value != 0. TODO(anon): Verify this assumption)
    ...
    , MSTORE                                                ⤳ S [V "gas", V "to", V "value", V "in offset", V "in size", V "out offset", V "out size"] -- TODO(anon): Should we give internal calls control over gas?
