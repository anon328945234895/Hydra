...
instrumentOps mc = concatMap aux
    where aux (Op STOP)         = [ Push 0, Op $ DUP 1, Push 1
          ...
          -- TODO(anon): All ops that offset memory are vulnerable to overflow (MLOAD, MSTORE, MSTORE8, CALLDATALOAD, ...)
...
-- TODO(anon): Ensure that instrumentation never increases MSIZE

-- TODO(anon): Unify constants between heads
...
procInit = Proc "init" [] "_" (Scope
           ...
           -- TODO(anon): Memory initialization?
...
procCalldataload = Proc "calldataload" ["offset"] "data" (Scope
                   -- TODO(anon): Correct, but inefficient!
...
-- TODO(anon): Check size of input against maximum input size
procCall = let selfCall = (Scope
           ...
           ,(IfElse (And (Lt (Lit 0) (Var "to")) (M.leq (Var "to") (Lit maxPrecompileAddress)))
                -- TODO(anon): check behaviour of precompiles when called with non-zero value
                (Scope [(checkOrErr errorPrecompileCalledWithNonzeroValue (Iszero (Var "value")))
                       -- TODO(anon): Should we really forward the gas value here?
